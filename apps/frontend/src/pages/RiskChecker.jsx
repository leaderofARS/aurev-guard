...
export default function RiskChecker() {
  ...
  async function generateProof(){
    ...
    if(!result) return;
    try{
      const agent = await getAgentDecision(address, result.riskScore ?? result.score);
      const masumiDecision = agent.data?.decision || agent.decision || 'APPROVED';
      const log = await contractLog({ address, riskScore: result.riskScore ?? result.score, masumiDecision });
          ...
          <CardContent className="space-y-4">
            <form onSubmit={checkRisk}>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  type="button"
                  variant="secondary"
                  onClick={()=>{ setAddress(localStorage.getItem('aurev_address')||''); }}
                >
                  Autofill
                </Button>
              </div>
            ...
            {unsignedTx && (
              <Card className="mt-4">
                ...
                <CardContent>
                  <pre className="text-xs break-all overflow-auto">{unsignedTx}</pre>
                  <div className="mt-2 flex gap-2">
                    <Button variant="secondary" onClick={()=>navigator.clipboard.writeText(unsignedTx)}>
                      Copy
                    </Button>
            ...
            {signedTx && (
              <Card className="mt-4">
                <CardHeader>
                  <CardTitle>Signed Transaction (simulated)</CardTitle>
                </CardHeader>
                <CardContent>
                  <pre className="text-xs break-all overflow-auto">{signedTx}</pre>
                </CardContent>
              </Card>
            )}
          </CardContent>
